#! /usr/bin/env python
# -*- coding: utf-8 -*-
"""synology dsm api request module"""
...
class SynologyDSMModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        uri_params = dict(
            url=f'{task_args["base_url"]}/{task_args["cgi_path"].strip("/")}/{ task_args["cgi_name"]}',
        ...
        if task_args["request_method"] == "POST":
            ...
            uri_params[
                "url"
            ] += f'?api={task_args["api_name"]}&version={task_args["api_version"]}&method={task_args["api_method"]}'
            if "api_params" in task_args:
                uri_params["url"] += f'&{  urlencode(task_args["api_params"])}'
        ...
        if result.get("failed", False) or (
            not result.get("json", {}).get("success", None)
        ):
            result["failed"] = True
